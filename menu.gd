...
func _refresh_continue() -> void:
	var prog := Relay.progress()
	if prog.has_save and prog.is_chapter_unlocked(prog.current_chapter):
...
func _on_continue() -> void:
	var prog := Relay.progress()
	Relay.game().pending_chapter_id = prog.current_chapter
	Relay.game().pending_level_id = prog.current_chapter
...
func _on_new_game() -> void:
	...
	Relay.game().pending_chapter_id = 0
	Relay.game().pending_level_id = 0
	get_tree().change_scene_to_file("res://scenes/main/Main.tscn")
